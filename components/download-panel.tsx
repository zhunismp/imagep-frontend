...
import { RefreshCw, FileImage, Download, Loader2 } from "lucide-react";
...
export default function DownloadPanel({ taskId }: { taskId: string }) {
    ...
    const onDownloadAll = useCallback(async () => {
        ...
        if (!completed.length) {
            toast.message("No completed files to download yet.", {
                richColors: true,
                position: "top-right",
            });
            return;
        ...
        try {
            toast.message(`Downloading ${completed.length} files…`, {
                richColors: true,
                position: "top-right",
            });
            ...
            for (let i = 0; i < completed.length; i++) {
                ...
                // small delay to make browsers happier
                if (i > 0) await new Promise((r) => setTimeout(r, 250));

                const ok = await forceDownload(f.signedUrl, f.filename);
                if (ok) okCount++;
                else openedTabCount++;
            }

            toast.success(
                `Done. Downloaded ${okCount}/${completed.length}` +
                (openedTabCount ? ` • Opened ${openedTabCount} in new tabs` : ""),
                { richColors: true, position: "top-right" }
            );
        } finally {
            setDownloadingAll(false);
        }
    }, [completed, downloadingAll]);
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                        {data.isCompleted && (
                            <button
                                ...
                                disabled={downloadingAll || completed.length === 0}
                                className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 text-white px-3 py-2 text-sm hover:bg-slate-800 disabled:opacity-60 disabled:cursor-not-allowed"
                            >
                                {downloadingAll ? (
                                    <>
                        ...
                        <button
                            ...
                            className="inline-flex items-center justify-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm hover:bg-slate-50 disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                            <RefreshCw className={`w-4 h-4 ${isFetching ? "animate-spin" : ""}`} />
                            Refresh
                        </button>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
                                {completed.map((f, i) => (
                                    <button
                                        ...
                                        className="animate-fade-in text-left rounded-2xl border bg-white p-3 hover:bg-slate-50 active:scale-[0.99] transition disabled:opacity-70 disabled:cursor-not-allowed"
                                    >
